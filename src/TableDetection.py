...
class DetectTable(object):
    ...
    def run(self):
        ...
        kernel_len = np.array(img).shape[1] // 5
            ...
            for j in range(len(finalboxes[i])):
                ...
                if(len(finalboxes[i][j]) == 0):
                    ...
                    for k in range(len(finalboxes[i][j])):
                        ...
                        out = pytesseract.image_to_string(erosion, lang='kor+eng', config='--psm 1 -c preserve_interword_spaces=1')  # OCR
                        if(len(out) == 0):
